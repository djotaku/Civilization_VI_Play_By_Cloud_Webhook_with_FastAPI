...
async def handle_play_by_cloud_json(play_by_cloud_game: CivTurnInfo):
    ...
    if player.matrix_username:
        player_name = player.matrix_username
    else:
        player_name = play_by_cloud_game.value2
    ...
    if continuing_game:
        ...
        await game_service.update_game(game_name=game_name, player_id=str(player.id), turn_number=turn_number,
                                       time_stamp=time_stamp, turn_deltas=turn_deltas,
                                       average_turn_time=average_turn_time)
    else:
        turn_deltas.append(time_since_last_turn)
        average_turn_time = get_average_time(turn_deltas)
        await game_service.create_game(game_name=game_name, player_id=str(player.id), turn_number=turn_number,
                                       time_stamp=time_stamp, turn_deltas=turn_deltas,
                                       average_turn_time=average_turn_time)
